...
export const apiDolibarr = {
  ...
  createSalaire: async (salaireData) => {
    ...
    try {
      ...
      // ✨ OPTIMISATION DASHBOARD : On récupère le genre de l'employé pour les stats locales
      let genreTrouve = 'homme';
      try {
        const employes = await apiDolibarr.getEmployes();
        const emp = employes.find(e => Number(e.id || e.rowid) === Number(payload.fk_user));
        if (emp) {
          genreTrouve = (emp.gender || emp.genre || 'homme').toLowerCase() === 'woman' ? 'femme' : 'homme';
        }
      } catch (empErr) {
        console.error("Impossible de lier le genre pour le salaire simulé", empErr);
      }
      ...
      const newSalary = { 
        ...
        ref_employe: payload.fk_user,
        date_debut: payload.datesp,
        date_fin: payload.dateep,
        montant: payload.amount,
        genre: genreTrouve, // Utilisé directement par le Dashboard si l'employé venait à manquer
        simulated: true,
        paiements: paiements
      };
  ...
  getSalaires: async () => {
    try {
      ...
      // Filtrer les salaires Dolibarr valides rattachés à un utilisateur existant
